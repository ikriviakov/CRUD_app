...
def change_status_to_Completed(self, request, queryset):
    ...
    self.message_user(request, f"Изменён статус на 'Завершено' у {queryset.count()} записи(ей).")

@admin.action(description='Поменять статус на "Новая"')
def change_status_to_New(self, request, queryset):
    queryset.update(status='New')
    self.message_user(request, f"Изменён статус на 'Новая' у {queryset.count()} записи(ей).")

@admin.action(description='Поменять статус на "В работе"')
def change_status_to_In_progress(self, request, queryset):
    queryset.update(status='In_progress')
    self.message_user(request, f"Изменён статус на 'В работе' у {queryset.count()} записи(ей).")

@admin.register(BugReport)
class BugReportAdmin(admin.ModelAdmin):
    list_display = ('title', 'created_at', 'project','task')
